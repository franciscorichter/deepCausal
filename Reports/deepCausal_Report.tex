...
\texttt{train\_causal()} accepts two environment datasets \texttt{data\_G1}, \texttt{data\_G2}, the tradeoff parameter \(\lambda\), the \texttt{target} column name, and optional \texttt{cv\_folds}. By default, parameters are optimized with an Adam optimizer using analytic gradients for both linear and neural models. If \texttt{optimizer="optim"} is chosen, the function falls back to \texttt{stats::optim}. If \texttt{cv\_folds > 1}, K-fold CV is run in each environment separately, models are fit on training folds, and performance is measured by RMSE on the pooled validation folds.
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.48\textwidth]{figures/sys2_linear_pred_vs_actual.png}\hfill
  \includegraphics[width=0.48\textwidth]{figures/sys2_nn_pred_vs_actual.png}
  \caption{System 2 predicted vs. actual on test set. Left: Linear. Right: NN.}
  \label{fig:sys2_pred_vs_actual}
\end{figure}

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.48\textwidth]{figures/sys2_linear_loss.png}\hfill
  \includegraphics[width=0.48\textwidth]{figures/sys2_nn_loss.png}
  \caption{System 2 training loss curves (Adam). Left: Linear. Right: NN.}
  \label{fig:sys2_loss}
\end{figure}

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.48\textwidth]{figures/sys2_linear_resid_hist.png}\hfill
  \includegraphics[width=0.48\textwidth]{figures/sys2_nn_resid_hist.png}
  \caption{System 2 residual histograms on test set. Left: Linear. Right: NN.}
  \label{fig:sys2_resid}
\end{figure}

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.48\textwidth]{figures/sys1_linear_pred_vs_actual.png}\hfill
  \includegraphics[width=0.48\textwidth]{figures/sys1_nn_pred_vs_actual.png}
  \caption{Predicted vs. actual on the System 1 test set. Left: Linear. Right: NN.}
  \label{fig:sys1_pred_vs_actual}
\end{figure}
...
\begin{itemize}
  \item \textbf{Linear model} (\(\lambda=0.3\), CV=5): \textbf{CV RMSE} \(\approx 1.1081\). Test \textbf{RMSE} \(\approx 1.7176\), \textbf{MSE} \(\approx 2.9500\).
  \item \textbf{Neural net} (hidden layers \(3,3\), \(\lambda=0.6\), CV=5): \textbf{CV RMSE} \(\approx 1.1633\). Test \textbf{RMSE} \(\approx 1.3176\), \textbf{MSE} \(\approx 1.7359\).
\end{itemize}

\subsection{System 2 (post-fix: no leakage)}
Environments differ in the noise scale \texttt{sd\_eps}. The generator sets \(Y:=X_5\) and drops \texttt{X5} to avoid leakage.
\begin{itemize}
  \item \textbf{Linear model} (\(\lambda=0.3\), CV=5): \textbf{CV RMSE} \(\approx 2.2440\). Test \textbf{RMSE} \(\approx 2.1023\), \textbf{MSE} \(\approx 4.4197\).
  \item \textbf{Neural net} (hidden layers \(3,3\), \(\lambda=0.3\), CV=5): \textbf{CV RMSE} \(\approx 128.47\). Test \textbf{RMSE} \(\approx 60.33\), \textbf{MSE} \(\approx 3640.08\).
\end{itemize}

\subsection{System 3}
\begin{itemize}
  \item \textbf{Linear model} (\(\lambda=0.3\), CV=5): \textbf{CV RMSE} \(\approx 1.6284\). Test \textbf{RMSE} \(\approx 1.4918\), \textbf{MSE} \(\approx 2.2254\).
  \item \textbf{Neural net} (hidden layers \(3,3\), \(\lambda=0.3\), CV=5): \textbf{CV RMSE} \(\approx 1.4590\). Test \textbf{RMSE} \(\approx 1.3455\), \textbf{MSE} \(\approx 1.8103\).
\end{itemize}

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.48\textwidth]{figures/sys3_linear_pred_vs_actual.png}\hfill
  \includegraphics[width=0.48\textwidth]{figures/sys3_nn_pred_vs_actual.png}
  \caption{System 3 predicted vs. actual on test set. Left: Linear. Right: NN.}
  \label{fig:sys3_pred_vs_actual}
\end{figure}

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.48\textwidth]{figures/sys3_linear_loss.png}\hfill
  \includegraphics[width=0.48\textwidth]{figures/sys3_nn_loss.png}
  \caption{System 3 training loss curves (Adam). Left: Linear. Right: NN.}
  \label{fig:sys3_loss}
\end{figure}

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.48\textwidth]{figures/sys3_linear_resid_hist.png}\hfill
  \includegraphics[width=0.48\textwidth]{figures/sys3_nn_resid_hist.png}
  \caption{System 3 residual histograms on test set. Left: Linear. Right: NN.}
  \label{fig:sys3_resid}
\end{figure}

\subsection{System 4}
\begin{itemize}
  \item \textbf{Linear model} (\(\lambda=0.3\), CV=5): \textbf{CV RMSE} \(\approx 2.7336\). Test \textbf{RMSE} \(\approx 2.4678\), \textbf{MSE} \(\approx 6.0900\).
  \item \textbf{Neural net} (hidden layers \(3,3\), \(\lambda=0.3\), CV=5): \textbf{CV RMSE} \(\approx 2.5808\). Test \textbf{RMSE} \(\approx 2.5117\), \textbf{MSE} \(\approx 6.3087\).
\end{itemize}

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.48\textwidth]{figures/sys4_linear_pred_vs_actual.png}\hfill
  \includegraphics[width=0.48\textwidth]{figures/sys4_nn_pred_vs_actual.png}
  \caption{System 4 predicted vs. actual on test set. Left: Linear. Right: NN.}
  \label{fig:sys4_pred_vs_actual}
\end{figure}

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.48\textwidth]{figures/sys4_linear_loss.png}\hfill
  \includegraphics[width=0.48\textwidth]{figures/sys4_nn_loss.png}
  \caption{System 4 training loss curves (Adam). Left: Linear. Right: NN.}
  \label{fig:sys4_loss}
\end{figure}

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.48\textwidth]{figures/sys4_linear_resid_hist.png}\hfill
  \includegraphics[width=0.48\textwidth]{figures/sys4_nn_resid_hist.png}
  \caption{System 4 residual histograms on test set. Left: Linear. Right: NN.}
  \label{fig:sys4_resid}
\end{figure}
